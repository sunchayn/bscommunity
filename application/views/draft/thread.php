    ...
    <div class="area sub-wrapper">
        <div class="grid-section">
            ...
            <form action="ajax/createThread" method="POST" class="ajax editor inboxDraft">
                ...
                <!-- end -  AJAX LOADER -->
                <div class="col-m col-12 box">
                    <input type="text" name="title" placeholder="<?=Language::invokeOutput('placeholder/thread');?>" class="col-12 draftTrigger" value="<?=$data['draft']->title;?>">
                </div>
                <div class="col-m col-12 box">
                    <textarea id="bseContentHolder" name="content" class="col-12 bse" rows="17" placeholder="<?=Language::invokeOutput('content/thread');?>"><?=$data['draft']->content;?></textarea>
                </div>
                <div class="col-m col-12 box">
                    <input type="text" name="keywords" class="col-8 draftTrigger" placeholder="<?=Language::invokeOutput('placeholder/keywords');?>" value="<?=$data['draft']->keywords;?>"><br />
                ...
                <div class="col-m col-12">
                    ...
                    <input type="hidden" name="attachments" class="attachmentsBus" value="<?=$data['draft']->attachments;?>" />
                    <input type="hidden" name="draft" value="<?=$data['draft']->id;?>">
                </div>
                <?php if (isset($data['draft']->attachments) && !empty($data['draft']->attachments)) {?>
                    <!-- #  attachments # -->
                    <div class="col-m col-12">
                        <small><?=Language::invokeOutput('curr-attach');?></small>
                        <div class="col-m col-12">
                            <?=attachmentAPI::fetchAttachments($data['draft']->attachments);?>
                        </div>
                    </div>
                    <!-- #  attachments # -->
                <?php } ?>
